...
| `PointOffset` | Enable offset point | Property | [`PointOffset`](common/PointOffset.md) |  |  | Define an offset point for the activity. Overrides target offset point (if defined). |
...
- Unlike other activities, the [offset point](common/PointOffset.md) is always measured from the top-left corner of the picture, never its center — including when no offset is set.
- `TopLeft` is the only accepted `PointOffset.Position`, on the activity and on the target alike. Any other value, including the `Center` default, is a validation error.
