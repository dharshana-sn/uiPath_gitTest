...
**Do not use `idx` in the filter.** In a `FilterTarget` fuzzy selector, `idx` resolves as an ordinal within the whole search scope, not per matching parent — `<ctrl role='cell' idx='2' />` pins the single second matching cell in the scope, so the activity returns one element instead of the collection and the run still completes without errors. When the returned collection looks truncated, remove every `idx` from the filter and discriminate by role, name, or other attributes instead.
