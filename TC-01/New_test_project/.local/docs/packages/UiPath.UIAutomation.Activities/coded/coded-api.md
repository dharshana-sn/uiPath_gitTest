...
`TargetAnchorableModel` — runtime-model equivalent of [`TargetAnchorable`](../activities/common/Target.md#targetanchorable): same selector, anchor, image, native text, semantic, and CV data as activity-side `TargetAnchorable`, but plain runtime object, not XAML sub-object with `InArgument<T>` properties. Most coded-API methods accept it via `UiTargetApp` indexers (`uiTargetApp["target name"]` or `uiTargetApp[descriptor]`) or built by the static `Target` factories from a CLI-captured selector, fuzzy selector, semantic selector, image, or CV data (`Target.FromSelector("<…/>")`, `WithScopeSelector`, `WithFuzzySelector`, …). Full property surface: [`Target.md`](../activities/common/Target.md).

`TargetAppModel` — runtime-model equivalent of [`TargetApp`](../activities/common/Target.md#targetapp), used by `Attach(TargetAppModel)` / `Open(TargetAppModel)` to scope an application by selector (`Selector`, `FilePath`, `Arguments`, `Url`, `BrowserType`, `IsExactTitleEnabled`) without an Object Repository screen. Rules and examples for both: [coded-authoring-guide.md § Selector-Only Targets](../references/coded-authoring-guide.md#selector-only-targets-no-object-repository). `RuntimeTarget` — opaque runtime handle (`Guid` reference) returned by `GetRuntimeTarget`, consumed by other methods.
...
`Attach` and `Open` return a disposable `UiTargetApp`. **Disposing it closes the application by default** (`TargetAppOptions.CloseMode` defaults to `Always`); pass `CloseMode = NAppCloseMode.Never` to leave the app running. `CloseMode` takes effect only when the `UiTargetApp` is disposed — at the end of the `using` block or on an explicit `Dispose()`/`Close()` call; until then the app stays open regardless of its value. When migrating from XAML, carry over the `NAppCloseMode` from the original Use Application/Browser activity — the coded default (`Always`) is not the XAML default (`IfOpenedByAppBrowser`), so an app the workflow only attached to would now be closed.

### Attach

Connects to an app/screen that is **already running** — never launches it (`appAttachMode` only controls which windows are searched for targets).
...
Launches the app/screen (`openMode`: `IfNotOpen` by default, `Always` for a new instance) or navigates to it. When migrating from XAML, keep the `NAppOpenMode` configured on the original Use Application/Browser activity. When writing a workflow from scratch, choose between `Open` and [`Attach`](#attach) based on the actual state of the application — it may already be running, e.g. launched by another workflow or a script.
...
| `PointOffset` | [`PointOffsetModel`](#pointoffsetmodel) | Offset point for the action. |
...
| `PointOffset` | [`PointOffsetModel`](#pointoffsetmodel) | Offset point for the source target. |
| `SecondaryPointOffset` | [`PointOffsetModel`](#pointoffsetmodel) | Offset point for the destination target. |
...
Runtime form of [`PointOffset`](../activities/common/PointOffset.md): `Position: NPosition` (default `NPosition.Center`), `X: int`, `Y: int`. Where to set it, and what happens when both the options object and the target carry one: [PointOffset.md](../activities/common/PointOffset.md#where-to-set-the-offset).
...
```csharp
// "MyWebApp" is already open/running — Attach only connects to it.
...
```csharp
// "LoginScreen" may not be running yet — Open launches it if not already open.
