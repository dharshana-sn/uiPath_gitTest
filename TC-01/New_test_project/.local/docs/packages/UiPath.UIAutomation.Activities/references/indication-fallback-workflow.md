...
- **Coded:** `ObjectRepository.cs` regeneration needs a `[Workflow]`/`[TestCase]` `.cs` on disk and one per-file `uip rpa validate` after indication. Rules and member-naming transforms: [coded-authoring-guide.md § Step 1](coded-authoring-guide.md), § Descriptor Naming. If missing/stale, confirm entries with `uip rpa get-object-repository --project-dir "<PROJECT_DIR>" --output json`; `Descriptors.*` still requires the generated file.
