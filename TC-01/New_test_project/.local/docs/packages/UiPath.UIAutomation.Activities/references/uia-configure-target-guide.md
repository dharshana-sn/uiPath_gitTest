...
**Execute steps inline in main conversation.** Never delegate the skill;
...
Read SKILL.md; execute every procedure step.
...
**Read it via a two-step read, never a plain full Read:** (1) Grep `^## Fallback Routes` on the exact SKILL.md path with line numbers (Grep on an explicitly named file works even under gitignored `.local/`; only directory-wide searches are blind there), (2) Read the file with `limit` set to that line. Everything above the marker is the mandatory procedure; the fallback routes below it (TARGET-9 CV, TARGET-10 semantic) are read IN FULL only when the flow routes an element there. If the Grep unexpectedly returns nothing, fall back to a full Read.

**Locate file:** fixed package path `{PROJECT_DIR}/.local/docs/packages/UiPath.UIAutomation.Activities/skills/uia-configure-target/SKILL.md`; open with direct `Read` — a failed Read is the existence check. Do NOT probe for the file's EXISTENCE with `Glob` or directory-wide `Grep`: both skip `.local/` during traversal, so a miss proves nothing. To list directory contents, `ls` the exact path via shell.

Installed package version in `project.json` is authoritative for availability — never a file-search miss.
...
- Produced selector still `NEEDS_IMPROVEMENT` after snapshot-retry and the 7.2 improvement pass.
