...
| `get-screen-definition` | Writes an existing screen's definition to a file. |
| `get-screen-xaml` | Gets XAML representation of screen. |
| `replace-screens` | Replaces multiple screens from definition files in single batch. |
...
| `get-element-definition` | Writes an existing element's definition to a file. |
| `get-elements-xaml` | Gets XAML representation of multiple elements in single batch. |
| `replace-elements` | Replaces multiple elements from definition files in single batch. |
| `delete-element` | Deletes element entry. |

Coded `ObjectRepository.cs` picks up writes made here on the next per-file `uip rpa validate`: [coded-authoring-guide.md § Step 1](coded-authoring-guide.md).
