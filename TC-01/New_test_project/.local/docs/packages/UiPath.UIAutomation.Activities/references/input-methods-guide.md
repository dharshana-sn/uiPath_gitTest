...
- **Special keys** (`[k(key)]`/`[d(key)]`/`[u(key)]` in `type`): use `HardwareEvents` or `DebuggerApi`; `Simulate` types the tokens literally. Click options (`--type`, `--button`, `--modifiers`) may work under `Simulate`.
- **Date/formatted date-time inputs:** use key events (`DebuggerApi` or `HardwareEvents`). `Simulate` sets value without validation-dependent input events. See `uia-elements-interaction-guide.md` § Date and time inputs in this `references/` folder.
