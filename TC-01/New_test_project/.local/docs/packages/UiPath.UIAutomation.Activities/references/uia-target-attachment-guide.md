...
### Linking Rewrites the Whole File

Both commands load the `.xaml` through the workflow designer and serialize it back, so the injected `.Target` / `.TargetApp` is not the only change. Attributes the designer treats as non-canonical are dropped — an authored `IsDisplayNameAuto="False"` on `NApplicationCard` does not survive. Attribute order changes on touched activities, and `xmlns` declarations left unused are removed. Expect a post-link diff larger than the injected targets.

Author the activities, link, then re-check anything cosmetic set by hand. Never re-add a dropped attribute in a loop: the linked file is authoritative, and the injected targets are the payload that matters.
