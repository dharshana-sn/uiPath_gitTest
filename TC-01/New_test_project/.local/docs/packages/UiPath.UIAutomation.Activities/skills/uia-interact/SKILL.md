...
"$CLI" interact drag e5 e9                                       # Drag element e5 and drop it onto e9 (left button, centers)
"$CLI" interact drag e5 e9 --source-offset-x 5 --destination-origin TopLeft # Per-side origin/offset: --source-* and --destination-*
...
"$CLI" interact focus e5                                         # Set keyboard focus to element
"$CLI" interact scroll-into-view e5                              # Scroll element into view
```
...
`get` and `get-all` print values double-quoted, so padding stays visible. The real value is the text between those outer quotes, padding included.
...
- **[state]** -- State markers: `[selected]`, `[focused]`, `[disabled]`, `[read only]`, `[invisible]`, `[offscreen]`, `[no rectangle]`, `[sap]`, `[items deferred]`, `[minimized]`, `[foreground]`; others can appear.
- **: text** -- Inline value (e.g., `InputBox [ref=e3]: pre-filled`)
- **/attr** -- Attributes as child lines (e.g., `/url`, `/placeholder`, `/tooltip`, `/automationid`, `/itemId`, `/data-*`, `/sapSysSessionId`)
- **Children** -- Nested with indentation

Top-level example (`uia snapshot`, no ref):

```
- Window "Hello World" [ref=w1] [minimized]:
  - /process: notepad.exe
  - /class: Notepad
...
- Every captured node carries a `[ref=...]`.
- `[disabled]` elements are not interactable -- skip.
- `[selected]` on CheckBox/RadioButton = checked; on TabPage/ListItem = active.
- `[invisible]` / `[offscreen]` = element is not currently displayed; `[no rectangle]` = element has no usable screen rectangle. Prefer duplicates without these states.
...
- Best-effort: any field (role, name, state, attribute, value) appears only when the framework reports it, so a missing field proves nothing -- a node with no `[focused]` may still be focused. Read live via `"$CLI" interact get`/`get-all <ref>` (tables: `extract-table <ref>`) when it matters.
...
**Region refs** — semantic (s1, s2...) from `snapshot semantic-find <w/b-ref>`, Computer Vision (cve1.../cvw1...) from `snapshot capture --type cv <w/b-ref>`. Each call replaces that kind's prior refs. Screen positions, not live elements: after layout change they may silently hit the wrong place.
...
Special keys in `interact type`: use HardwareEvents or DebuggerApi; Simulate types the tokens literally. Click options (`--type`, `--button`, `--modifiers`) may work under Simulate.
...
- `[discarded]` -- Suspended by browser to save memory. Any capture or interaction on the tab restores it first; if it stays unloaded, `uia interact browser tab-select <b-ref>`, or control the window as a desktop application via its w-ref.
...
- `[extension update required]` -- Installed browser extension must be updated to automate multiple browser profiles.
- `[extension incompatible]` -- Browser extension is incompatible with the installed `UiPath.UIAutomation.Activities` version, or the browser's DeveloperToolsAvailability policy is not set to 1.
- `[IE mode blocked]` -- Edge IE mode tab cannot be accessed through the browser extension.
- `[IE protected mode]` -- Internet Explorer is running in an AppContainer environment, possibly with Enhanced Protected Mode enabled.
- `[requires elevation]` -- Browser runs elevated while the automation process does not.

In most cases, the w-ref still controls the browser as a desktop application.
